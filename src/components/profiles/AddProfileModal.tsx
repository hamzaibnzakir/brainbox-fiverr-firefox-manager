...
export function AddProfileModal({
  ...
  onCreate: (p: FirefoxProfile) => void;
  ...
  const [draft, setDraft] = useState<DraftState>(() => makeEmpty());
  ...
  function finish() {
    const newProfile: FirefoxProfile = {
      id: `p-${Date.now()}`,
      accountName: draft.accountName || draft.firefoxProfileName,
      countryCode: draft.countryCode || "us",
      countryName: draft.countryName || "Unspecified",
      status: "stopped",
      proxyHost: draft.proxyHost,
      proxyPort: Number(draft.proxyPort) || 0,
      proxyUsername: draft.proxyUsername,
      proxyPassword: draft.proxyPassword,
      proxyStatus: "healthy",
      latencyMs: 0,
      externalIp: draft.proxyHost,
      firefoxProfileName: draft.firefoxProfileName,
      targetWebsite: draft.targetWebsite,
      lastLaunched: "Never",
  ...
  const canAdvance =
    (step === 0 && draft.firefoxProfileName && draft.accountName) ||
    (step === 1 && draft.proxyHost && draft.proxyPort) ||
    (step === 2 && draft.targetWebsite);
  ...
  return (
    <Dialog
      ...
      title="Add profile"
      description="Configure a new isolated Firefox identity."
      widthClass="max-w-lg"
    >
      ...
      {step === 0 && (
        <div className="flex flex-col gap-4">
          <Select
            label="Discovered Firefox profiles"
            value={draft.firefoxProfileName}
            onChange={(v) => set("firefoxProfileName", v)}
            options={discoveredProfiles.map((p) => ({
              value: p.name,
              label: p.name,
              hint: p.inUse ? "in use" : "available",
            }))}
          />
          <Input
            label="Account name"
            placeholder="e.g. USA Fiverr"
      ...
      {step === 2 && (
        <div className="flex flex-col gap-4">
          <Input
            ...
            placeholder="fiverr.com/users/manage_gigs"
            value={draft.targetWebsite}
      ...
      <div className="mt-6 flex items-center justify-between">
        ...
        {step < STEPS.length - 1 ? (
          ...
          <Button variant="primary" disabled={!canAdvance} onClick={finish}>
            Finish
          </Button>
        )}
      </div>
