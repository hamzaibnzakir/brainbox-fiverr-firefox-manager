...
const FALLBACK: AppSnapshot = {
  ...
  settings: { workspaceName:"Brainbox", firefoxBinary:"", launchOnLogin:false, launchAllAuto:true, startupDelay:12, defaultSite:DEFAULT_SITE, theme:"dark", notifyProxyFail:true, notifyLaunch:false, refreshEnabled:false, refreshMinMinutes:5, refreshMaxMinutes:15 },
};
...
export function AppDataProvider({ children }: { children: React.ReactNode }) {
  ...
  const refresh = useCallback(async () => {
    try { await apply(api.snapshot); } catch { /* keep current state */ } finally { setLoading(false); }
  }, [apply]);

  useEffect(() => {
    void refresh();
    const timer = window.setInterval(() => {
      void api.snapshot().then(setData).then(() => setError(null)).catch(() => {});
    }, 2500);
